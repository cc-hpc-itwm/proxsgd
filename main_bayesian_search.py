# This file is used for hyperparameter search using Tree Structured Parzen Estimator based on Bayes 
# theorem. 'config.py' consist all the variables information such as result directory, dataset, 
# training epochs , retraining epochs, network and so on. 
# To run the trials, create a study object, which sets the direction of optimization 
# ("maximize" or "minimize"), along with other settings.
...
def objective(trial):
    ...
    result_accuracies_for_each_run.append(pruned_compression_rate.split('x',1)[0])
    result_accuracies_for_each_run.append(pruned_compression_rate.split('x',1)[1])
...
if __name__ == "__main__":
    ...
    with open(config.accuracy_comparison_dir+config.csv_filename, 'a') as f:
        writer = csv.writer(f)
        writer.writerow(['Run_id','Original_Accuracy','compression_rate_2.0','Percentage Pruned']) # Write column names to the csv file.
        f.close()              
    ...
    study = optuna.create_study(study_name=config.study_name,storage=storage,direction='maximize', load_if_exists=True,sampler=TPESampler()) #Create study to maintain history in specified storage and other settings.
