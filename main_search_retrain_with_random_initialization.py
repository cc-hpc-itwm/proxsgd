# This file is used for hyperparameter search using TPE(Tree Structured Parzen Structure) Search. 
# 'config.py' consist all the variables information  such as result directory, dataset, training epochs , 
# retraining epochs, network and so on. After search network is pruned and then remaining weights get initilaized
# with Xavier weight initialization for retraining.

#To run the trials, create a study object,  which sets the direction of optimization ("maximize" or 
# "minimize"), along with other settings.
...
def objective(trial):
    ...
    rho_decay=trial.suggest_float('rho_decay', 0.5, 0.8, log=True)
    epsilon_decay=trial.suggest_float('epsilon_decay', rho_decay, 0.8, log=True)
    ...
    #trained_model_path='/home/shalinis/proxsgd_random_search/results/experiment_densenet_CIFAR100_epsilon_0.11739629629055848_epsilon_decay_0.5592710641770795_rho_0.5869814814527924_rho_decay_0.5556043477210715_mu_1e-05/train/BEST_checkpoint.pth.tar'
    ...
    retraining_accuracies_for_each_run=[run_id]
    retraining_accuracies_for_each_run.append(original_model_acc1)

    best_acc=0
    #Prune Trained model
    for i in range(len(config.threshold)):
        checkpoint = torch.load(trained_model_path)
        trained_model=checkpoint['model']
        pruned_threshold= config.threshold[i] 
        pruned_model=prune_model(trained_model,pruned_threshold)
        print('\nPruned Model Evaluation for Threshold value: ', pruned_threshold)
        pruned_model_loss, pruned_model_acc1, pruned_model_acc5 = test(pruned_model,test_loader,logger)
        
        
        #Weight initialization of pruned model for retraining
        for index,module in enumerate(pruned_model.modules()):
        ...
        retrained_model_loss, retrained_model_acc1, retrained_model_acc5 = test(retrained_model,test_loader,logger)
         
        retraining_accuracies_for_each_run.append(retrained_model_acc1)
        
        if retrained_model_acc1 > best_acc:
            best_acc=retrained_model_acc1
            trial.set_user_attr('threshold', pruned_threshold)
...
if __name__ == "__main__":
    ...
    with open(config.accuracy_comparison_dir+config.csv_filename, 'a') as f:
        writer = csv.writer(f)
        writer.writerow(['Run_id','Original_Accuracy','Retraining_Accuracy_Threshold_2.0','Retraining_Accuracy_Threshold_2.5']) # Write column names to the csv file.
        f.close()              
    ...
    print('Retrained Accuracy: {}'.format(trial.value))
